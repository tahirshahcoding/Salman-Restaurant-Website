    ...
    <>
      <section id="home" className="relative bg-white pt-28 pb-0 md:pt-36 overflow-hidden min-h-screen flex flex-col justify-center">
        ...
        <div className="w-full max-w-[1750px] mx-auto px-4 sm:px-6 lg:px-12 xl:px-16 grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 items-center relative z-10 flex-1">
          ...
          {/* ── Image Column ───────────────────────────────────────── */}
          <motion.div
            className="lg:col-span-6 xl:col-span-5 relative w-full h-[450px] sm:h-[600px] lg:h-[780px] xl:h-[880px] flex items-end justify-center z-10"
            initial={{ opacity: 0, x: 60 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.9, delay: 0.3, ease: [0.22, 1, 0.36, 1] }}
          >

            {/* Red Brush Stroke Background */}
            <img
              src={brushImg}
              alt="Brush Stroke Accent"
              className="absolute right-0 bottom-[-20px] lg:bottom-[-80px] w-[100%] sm:w-[85%] lg:w-[130%] xl:w-[145%] max-w-none h-auto object-contain z-0 select-none pointer-events-none transform translate-y-4 lg:translate-y-12"
            />
